...
| **Basket** | The Hive's store of bytes worth keeping that Honey cannot hold: built binaries, wheels, container images, datasets, fixtures, model weights. Named for the pollen basket on a bee's leg. Content-addressed and signed, found through a Honey record (title, summary, hash) and never a listing, resident per Cell within a ceiling, carried between Cells over Waggle through the Hive Stand. A blob is inert: holding one grants no permission to run it; only the Comb makes one a tool |
...
| **Cell Wax** | Queen-written cautions about one Cell: a known limit, a risk, a quirk, each with a severity (note, caution, block), an optional expiry and a clearance label. Proposed by any bee, a Warden, the Patrol or the human; written and cleared only by the Queen; pulled into a prompt only when that Cell is a candidate for placement or assignment. Named for the wax bees use to cap and mend a cell: it marks the cell, it is not the honey inside |
...
- On a Real Cell with no active bees, drops into **watch mode**: a read-only script that observes what the device's access level allows, and once per Patrol interval wakes to review what it saw, raising an Alarm, filing a summary or proposing Cell Wax for the device if something matters, and discarding it if nothing does. It never writes to the device and never captures the screen or keyboard.
...
Which path the Queen takes is a per-task decision made from the task's needs (isolation, a display, a particular OS, network reach), the Forage each Cell can bear, any Cell Wax on the candidates (a block excludes a Cell, a caution counts against it), and the operator's preference: reuse a Real Cell already at hand when one fits, or provision a Virtual Cell when isolation, a clean image, or disposability matters more.
...
- **Hot state**: active goals, open tasks and Alarms, pending questions, recent decisions with their reasons, a summary of the fleet and Forage, pinned facts, short notes, and the Cell Wax of any Cell in play. Derived from the Brood Chamber and the Pheromone Trail; always loaded; bounded by construction, because it is packed by relevance until the budget fills.
- **Cell Wax**: Queen-written cautions about a Cell, kept in the memory tables and pulled into hot state only when that Cell is being considered, so a note about a flaky device costs nothing until the device is a candidate. A block keeps the Queen from placing work on the Cell until she clears it. Cleared and expired wax is ripened into Honey so the Cell's history compounds.
...
Beside the Comb sits the **Basket**, named for the pollen basket on a bee's leg. It holds the bytes that work produces and Honey cannot represent: a compiled binary, a wheel set, a container image, a dataset, a fixture screenshot, the weights a Nuc will serve. Every blob is content-addressed, signed by the Hive key at deposit, labelled with a clearance from its provenance, and described by one Honey record: a title and a short summary the bee supplies when it asks to keep the file, plus the hash that locates it. That record is how a blob is found, the same way anything else is: retrieval returns the few records that match a task, and no bee ever sees a listing, so a Basket of a thousand blobs costs a prompt nothing. The canonical copy lives on the Hive Stand; each Cell keeps the blobs its Warden chose to have resident, within a disk ceiling the Queen set, and a blob travels to another Cell over Waggle through the Hive Stand, verified on arrival. The rule that keeps it safe is that a blob is inert: holding one grants no permission to run it. Running a script from the Basket is an ordinary capped command, installing a program from it is an outside-scratch write with its restore path, and something is a tool only once the Comb has promoted it, which may name a Basket blob by hash as its package. Nothing enters the Basket by side effect: a bee proposes keeping an artefact, its Warden deposits it with provenance, and the trail records the deposit.
...
- **Fleet list**: every Cell, filterable to Real only, Virtual only, or all, with what each is doing, its **Comb Shield level**, its Pheromone Mask state, its access level and mode, a Cell Wax indicator showing the highest live severity on it, and where its models are hosted, on the Hive Stand, a hosted API, or on the machine itself.
