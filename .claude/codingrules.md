...
| Virtual Cell | Hive, `hive/cell_state.py` | `PROVISIONING → READY → GRANTED → RELEASED → DORMANT → READY`; `READY / GRANTED / RELEASED / DORMANT → DESTROYING → DESTROYED`; `PROVISIONING → FAILED`; a `NIGHT_VEIL` Cell never enters `DORMANT` | Backend labels are the source of truth; the table is reconciled against them. |
