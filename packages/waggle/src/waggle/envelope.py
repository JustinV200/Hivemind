...
PROTOCOL_VERSION = "1.4"  # What wrap() stamps: PROTOCOL_MAJOR.PROTOCOL_MINOR as the wire string.
PROTOCOL_MAJOR = 1  # A receiver rejects any other major; breaking changes bump it.
PROTOCOL_MINOR = 4  # Additive changes bump it; any minor of a known major is accepted. Bumped to
...
# diff cannot carry a binary, so a proposal may instead name a COPY by digest).
