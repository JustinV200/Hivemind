...
Waggle is the Hive's bee-to-bee wire protocol (named after the honeybee waggle dance). Its
control family is protocol housekeeping plus Hive-wide orders; the three messages here are the
Hive-wide half, split out of ``waggle.messages.control`` by responsibility so each file stays
under the codingrules 5.1 size limit. ``HumanMessage`` carries the human's own words into the
Hive, from the device they spoke on into the Queen's (the central orchestrator's) inbox or on to
the Warden (the always-on supervisor of one Cell, a unit of compute) of the task it concerns.
``MaskOverride`` forces or clears a Pheromone Mask (a short-lived behaviour overlay that makes a
bee write or move like a human) at the scope of one Cell. ``QueenMoved`` is the notice that the
Hive Stand (the machine the Queen runs on) has moved, signed with the Hive key, and the one
address that ever crosses Waggle. Every bound is a named constant here; the number, not the name,
is normative.
...
See Also:
    ...
    - waggle.messages.control for the family's other six messages and ClusterCause.
