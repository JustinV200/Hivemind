...
existing devices borrowed for tasks and left exactly as found). The three messages here concern
a colonized device, one whose Warden (the always-on supervisor of one Cell, a unit of compute)
lives on it, split out of ``waggle.messages.swarm`` by responsibility so each file stays under
the codingrules 5.1 size limit. The Queen (the central orchestrator) orders that Warden to start
or stop a local model server with a ``NucPromote``, making the device a Nuc (a colonized Real
Cell with its own model server, which keeps working when disconnected) or an ordinary colonized
Cell again, and the Warden answers with a ``NucPromoted``. When such a Warden reconnects after
working offline, it ships its local segment of the Pheromone Trail (the append-only audit log,
one logical log made of per-node segments) to the Queen in ``TrailSegmentSync`` chunks, which
the Queen merges idempotently. Every bound is a named constant here; the number, not the name,
is normative.

Fits into the Hive:
    Its own layer (used by every layer in hivemind and by pollen, the lightweight device
    connector), inside the waggle package. Registered by waggle.messages.registry, which maps
    each class to its kind; built by the Queen (promote) or a Warden (promoted, trail sync) and
    read by the other; calls into waggle.messages.base and waggle.messages.swarm (the family's
    enums) only.
...
See Also:
    ...
    - waggle.messages.swarm for EnrolRequest, EnrolAccept, DeviceHeartbeat and the enums.
