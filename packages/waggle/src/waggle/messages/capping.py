...
``waggle.messages.capping_action`` and the gate's word after the checks (``Verdict``,
``PostconditionResult``, ``RollbackDone``) in ``waggle.messages.capping_verdict``, split out by
...
Fits into the Hive:
    ...
    and waggle.messages.capping_action only.
...
See Also:
    ...
    - waggle.messages.capping_action for ProposedAction and ActionKind.
    - waggle.messages.capping_verdict for Verdict, PostconditionResult and RollbackDone.
...
from waggle.messages.capping_action import ProposedAction
