...
its ceilings and the chains of a hosting plan) live in ``waggle.messages.forage_capacity``, and
its messages in ``waggle.messages.forage`` and ``waggle.messages.forage_hosting``, each split
...
Fits into the Hive:
    ...
    connector), inside the waggle package. Imported by waggle.messages.forage (the grant
    messages) and waggle.messages.forage_capacity (SourceRef, for a chain); built by the Queen's
...
See Also:
    ...
    - waggle.messages.forage_capacity for the family's capacity and plan values.
    - waggle.messages.forage and waggle.messages.forage_hosting for the messages that carry
      these values.
"""
