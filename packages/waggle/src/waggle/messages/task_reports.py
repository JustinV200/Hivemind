...
to the Warden (the always-on supervisor of one Cell, a unit of compute) of the placed Cell, run
by a Worker (a sub-bee spawned for one task), reported on, and closed. The two messages here
travel back up the tree, Worker to Warden and Warden to Queen, and correlate to the
``TaskAssign`` they answer: ``TaskProgress`` reports a stage change, carrying the Handoff (the
document a bee writes before its context is reset) when the stage is a checkpoint, and
``TaskResult`` closes an attempt with its outcome, artifacts and spend, on two hops: an
unverified claim from the Worker, then the Warden's verified result after the acceptance checks.
Neither ever carries a transcript; a summary is a paragraph. The downward half of the family
(``TaskAssign`` and the orders on a task in flight) lives in ``waggle.messages.task``, from
which the attempt bound is imported so both halves count attempts alike. Every other bound is a
...
Fits into the Hive:
    ...
    each class to its kind; built by every Worker and Warden and read by every Warden and the
    Queen; calls into waggle.messages.base, waggle.messages.labels and waggle.messages.task only.
...
See Also:
    ...
    - waggle.messages.task for TaskAssign, whose attempt every report echoes, and the orders.
...
from waggle.messages.task import MIN_ATTEMPT
