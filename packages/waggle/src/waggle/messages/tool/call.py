...
promoted (or quarantining) tool version on a Cell (a unit of compute) with a JSON argument document,
through that Cell's Warden (its always-on supervisor) and its session; ``ToolResult`` answers with
the call's bounded JSON output or a stable error code, and its duration. The arguments and the
output are the family's sanctioned JSON text on the wire: each is bounded, proven to parse by
``waggle.messages.tool.json_text``, and validated against the tool's schema at the edge, never
handled as a mapping in transit. The authoring half (``ToolRequest``, ``ToolPromoted``) lives in
``waggle.messages.tool.authoring``, from which this module takes the version shape; the split is by
...
Fits into the Hive:
    ...
    waggle.messages.labels, waggle.messages.tool.authoring and waggle.messages.tool.json_text only.
...
See Also:
    ...
    - waggle.messages.tool.authoring for ToolRequest, ToolPromoted and the version shape.
    - waggle.messages.tool.json_text for the JSON checks on arguments_json and output_json.
...
from waggle.messages.tool.authoring import MAX_VERSION_CHARS, VERSION_PATTERN
from waggle.messages.tool.json_text import check_json_object_text, check_json_value_text
