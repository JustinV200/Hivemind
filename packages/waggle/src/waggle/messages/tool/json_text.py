...
Waggle is the Hive's bee-to-bee wire protocol (named after the honeybee waggle dance). Tool schemas,
arguments and outputs are inherently open JSON, so the tool family is the one sanctioned use of JSON
text on the wire (spec section 8): each such field is a bounded ``str`` that must parse as a JSON
object (a schema, an argument document) or as any JSON value (an output), and is validated against
the tool's schema at the edge (the Warden that runs the call, one of the always-on supervisors of a
Cell, a unit of compute), never handled as a mapping in transit. The two checks here prove the text
parses and return it unchanged, so the parsed value is dropped on the spot: a ``dict[str, Any]``
never leaves the codec that produced it (codingrules section 9), and a message field is text end to
end. They are split out of ``waggle.messages.tool.authoring`` by responsibility so each file stays
under the codingrules 5.1 size limit.

Fits into the Hive:
    Its own layer (used by every layer in hivemind and by pollen, the lightweight device connector),
    inside the waggle package. Imported by waggle.messages.tool.authoring (ToolPromoted's schema)
    and waggle.messages.tool.call (ToolInvoke's arguments, ToolResult's output) as AfterValidators
    on their text fields; calls into the standard library's json only.
...
See Also:
    ...
    - waggle.messages.tool.authoring and waggle.messages.tool.call for the fields that carry them.
"""
