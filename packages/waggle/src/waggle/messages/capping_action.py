...
split out of ``waggle.messages.capping`` by responsibility so each file stays under the
codingrules 5.1 size limit. Every bound is a named constant here; the number, not the name, is
normative.

Fits into the Hive:
    Its own layer (used by every layer in hivemind and by pollen, the lightweight device
    connector), inside the waggle package. Imported by waggle.messages.capping, whose
...
See Also:
    ...
    - waggle.messages.capping for ProposalSubmitted, which carries this action.
