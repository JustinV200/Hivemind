...
Wardens, sub-bees). The value models here are the bounded self-description every level sends
up: ``ContextTelemetry`` (what every bee reports on heartbeat, bounded so it can never smuggle a
transcript), ``ChildTelemetry`` (one sub-bee's row in a Warden's aggregate heartbeat; a Warden is
the always-on supervisor of one Cell, a unit of compute) and ``CompactView`` (the compacted view
an inspection returns, shaped after the sections of a Handoff, the document a bee writes before
its context is reset). ``WorkerState`` and ``WardenState`` are the wire forms of the two bee
state machines that ride on those rows. The messages that carry them (heartbeat, inspect and its
reply, intervene) live in ``waggle.messages.supervision``; the models are split out here by
...
Fits into the Hive:
    ...
    connector), inside the waggle package. Imported by waggle.messages.supervision, whose
...
See Also:
    ...
    - waggle.messages.supervision for Heartbeat, Inspect, InspectReply and Intervene.
