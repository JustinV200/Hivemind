...
move whole files across that link, split out of ``waggle.messages.session.commands`` by
responsibility so each file stays under the codingrules 5.1 size limit. ``SessionPutFile`` carries
one chunk of a file to the device, the chunks tied together by a ``transfer_id`` and closed by a
final chunk carrying the whole file's digest; ``SessionGetFile`` asks for a file, which comes back
as ``SessionOutput`` FILE chunks. Both end in one ``SessionExit`` (or a ``control.error``), as the
session rule of spec section 3 fixes, and both are recorded on the Pheromone Trail (the append-only
audit log) when they touch anything outside the lease's scratch directory. Every bound is a named
...
See Also:
    ...
    - waggle.messages.session.commands for the enums and the four Warden-sent messages.
    - waggle.messages.session.output for the SessionOutput and SessionExit that answer these.
