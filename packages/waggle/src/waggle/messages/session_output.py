...
are what the packet sends back, split out of ``waggle.messages.session`` by responsibility so
each file stays under the codingrules 5.1 size limit: ``SessionOutput`` streams one chunk of an
exec's stdout or stderr, or of a file being read, and ``SessionExit`` is the terminal event of
every session request (an open, an exec, a put-file or a get-file), the one message the session
rule of spec section 3 says each request ends in. Because an exit's ``request_kind`` is on the
model, every rule about which fields it carries is decidable without the request it answers.
...
Fits into the Hive:
    ...
    waggle.messages.base and waggle.messages.session (the family's enums) only.
...
See Also:
    ...
    - waggle.messages.session for the enums, EnvVar and the four Warden-sent messages.
    - waggle.messages.session_files for the transfers whose exits carry a size and digest.
