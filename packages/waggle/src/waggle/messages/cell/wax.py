...
Waggle is the Hive's bee-to-bee wire protocol (named after the honeybee waggle dance). Cell Wax is a
Queen-written caution about one Cell (a unit of compute: a Virtual Cell the Hive provisions, or a
Real Cell, an existing device borrowed for a task and left exactly as found): anyone may propose
one, only the Queen (the central orchestrator) writes or clears it, and placement (the Queen's
choice of a Cell for a task) treats a BLOCK as exclusion and a CAUTION as a penalty. The three
messages here are that note's life: ``CellWaxProposed`` (any bee to the Queen; a rejection is a
control.error with code hive.memory.wax_rejected), ``CellWaxWritten`` (the full note to the Cell's
Warden, its always-on supervisor, so it holds it even when offline) and ``CellWaxCleared`` (cleared,
expired on the sweep, or retired with its destroyed Virtual Cell). A note is identified by a
``wax_``-prefixed ULID string until an IdKind for it exists. The family's other messages live in
``waggle.messages.cell.status`` and ``waggle.messages.cell.leases``, split out by responsibility so
every file stays under the codingrules 5.1 size limit. Every bound is a named constant here; the
