...
directory passes: propose, check, apply, verify, roll back. This module is what the gate says
once the checks have run. ``Verdict`` is its final word on a proposal (a ``VerdictOutcome``:
verified, rejected, rolled back or changes requested, with what failed); ``PostconditionResult``
reports one postcondition, or one of a task's acceptance criteria, checked after apply by someone
other than the proposer; ``RollbackDone`` reports that an applied proposal was rolled back, by
which ``RollbackMethod``, whether completely, and what residue remains. The gate's entry
(``ProposalSubmitted``, ``CheckResult``) lives in ``waggle.messages.capping``, from which this
module takes ``CheckKind``; the split is by responsibility so each file stays under the
...
Fits into the Hive:
    ...
    waggle.messages.base, waggle.messages.labels and waggle.messages.capping only.
...
See Also:
    ...
    - waggle.messages.capping for ProposalSubmitted, CheckResult and CheckKind.
...
from waggle.messages.capping import MAX_POSTCONDITIONS, CheckKind
