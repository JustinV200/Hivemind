...
family moves knowledge in and out of the Honey Store (the persistent knowledge base), and a query's
answer is a ranked list of hits. ``HoneyHit`` is one such result: the store's row key, a title and a
bounded excerpt, a score, the scope it was found in, its clearance (the data-sensitivity label every
quoted datum carries), the tier of the Cell (a unit of compute) it originated on, and its
``HoneyProvenance`` (the task, Cell and bee it came from and when it was observed). The two value
models are split out of ``waggle.messages.honey.exchange`` by responsibility so each file stays
under the codingrules 5.1 size limit; ``SCOPE_PATTERN`` lives here because both a hit and a query
name scopes in the same shape. Every bound is a named constant here; the number, not the name, is
normative.

Fits into the Hive:
    Its own layer (used by every layer in hivemind and by pollen, the lightweight device connector),
    inside the waggle package. Imported by waggle.messages.honey.exchange, whose HoneyResponse
...
See Also:
    ...
    - waggle.messages.honey.exchange for NectarDeposit, HoneyQuery and HoneyResponse.
