...
live in ``waggle.messages.forage.values`` and the messages in ``waggle.messages.forage.grants`` and
``waggle.messages.forage.hosting``, split out by responsibility so every file stays under the
codingrules 5.1 size limit. Every bound is a named constant here; the number, not the name, is
normative.

Fits into the Hive:
    Its own layer (used by every layer in hivemind and by pollen, the lightweight device
    connector), inside the waggle package. Imported by waggle.messages.forage.hosting; built by
    ...
    waggle.messages.forage.values (SourceRef and the shared id bounds) only.
...
See Also:
    ...
    - waggle.messages.forage.values for SourceRef and the grant-side values.
...
from waggle.messages.forage.values import MAX_MODEL_CHARS, MAX_SOURCE_ID_CHARS, SourceRef
