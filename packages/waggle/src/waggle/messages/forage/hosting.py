...
Waggle is the Hive's bee-to-bee wire protocol (named after the honeybee waggle dance). Forage is the
Hive's capacity as data, and the four messages here are about where one Cell's (a unit of compute's)
models come from. ``CapacityReport`` is the Cell's own snapshot, sent by its Warden (its always-on
supervisor) or its Pollen Packet (the thin gateway on an enrolled device); live capacity travels
only here, never on a heartbeat. The other three are Queen (the central orchestrator) decisions with
their reasons: ``CeilingsSet`` bounds what a Warden may use of its own Cell without asking,
``HostingDecided`` records the mode (shared only, local first, local only) and ``PlanWritten``
writes the plan the Fanner (the seat meter every model call passes through) follows: per slot a
primary source and a fallback chain, plus a default chain. Each carries a revision so a replayed
older decision never overrides a newer one, a rule the receiver applies. The family's grant messages
live in ``waggle.messages.forage.grants`` and its value models in ``waggle.messages.forage.values``
and ``waggle.messages.forage.capacity``, split out by responsibility so every file stays under the
...
Fits into the Hive:
    ...
    waggle.messages.base, waggle.messages.reports, waggle.messages.forage.capacity and
    waggle.messages.forage.values only.
...
See Also:
    ...
    - waggle.messages.forage.grants for GrantIssued, GrantRevoked, ForageRequest and ForageReply.
    - waggle.messages.forage.capacity for the value models and enums these messages carry.
