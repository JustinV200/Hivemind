...
def test_max_attempts_below_one_is_refused(plain_codec: Codec, fake_clock: FakeClock) -> None:
    with pytest.raises(ValueError, match="max_attempts"):
        WebSocketClientTransport(
            "ws://127.0.0.1:9000", plain_codec, fake_clock, options=DialOptions(max_attempts=0)
        )
...
async def test_backoff_follows_the_documented_sequence_then_connect_failed(
    ...
    client = WebSocketClientTransport(
        uri, plain_codec, clock, options=DialOptions(max_attempts=4, open_timeout_s=DIAL_TIMEOUT_S)
    )
...
async def test_backoff_is_capped_at_reconnect_max(plain_codec: Codec) -> None:
    ...
    client = WebSocketClientTransport(
        ...
        plain_codec,
        clock,
        options=DialOptions(max_attempts=CAPPED_ATTEMPTS, open_timeout_s=DIAL_TIMEOUT_S / 4),
    )
...
async def test_connect_is_a_no_op_while_connected_and_dials_afresh_after_close(
    ...
    client = WebSocketClientTransport(
        server.uri, plain_codec, FakeClock(), options=DialOptions(max_attempts=1)
    )
    try:
        async with asyncio.timeout(WAIT_S):
            await client.connect()
...
async def test_reconnect_after_the_server_restarts_on_the_same_port(
    ...
    port = first.port
    client = WebSocketClientTransport(
        first.uri, plain_codec, FakeClock(), options=DialOptions(max_attempts=1)
    )
    async with asyncio.timeout(WAIT_S):
        await client.connect()
...
def test_gateway_host_is_accepted_only_with_the_virtual_cell_flag(
    ...
    with pytest.raises(ValueError, match="loopback"):
        WebSocketClientTransport(uri, plain_codec, fake_clock)
    client = WebSocketClientTransport(
        uri, plain_codec, fake_clock, options=DialOptions(allow_virtual_cell_gateway_host=True)
    )
