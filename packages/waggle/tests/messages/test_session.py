...
Fits into the Hive:
    ...
    rules are pinned in test_session_output.py and test_session_files.py.

Key invariants:
    - EXAMPLES holds exactly one instance of each of the eight session classes.

See Also:
    - waggle.messages.session, session_output and session_files for the modules under test.
...
def test_chunk_is_bounded_by_the_shared_chunk_limit(example: WaggleMessage) -> None:
    # SessionPutFile's chunk is pinned in test_session_files.py, where its total can grow too.
