...
by one lock, and applies the same three guards
``hivemind.entrance.store.sqlite.SqliteEntranceStore`` applies
(``hivemind.entrance.store.protocol``: entry, transition, single use), plus the same referential
rules the SQLite schema enforces (an invite names an existing, INVITED device, and at most one
invite per device), so the contract suite runs unchanged over both.
...
Fits into the Hive:
    ...
    into ``hivemind.entrance.enrol``, ``hivemind.entrance.errors`` and the protocol's guards.
