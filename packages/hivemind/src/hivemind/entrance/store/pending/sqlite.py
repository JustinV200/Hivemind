...
``settle_pending`` and writes the result, so of two people confirming the same request exactly one
wins, even from two processes.
...
class SqlitePendingTable:
    ...
    async def put(self, pending: PendingConfirmation) -> None:
        ...
        await self._link.run(_insert, pending)
    ...
    async def settle(
        ...
        # Blocking, sub-millisecond: one local statement or transaction on the link's thread.
        return await self._link.run(_settle, pending_id, expected, settlement)
...
def _insert(connection: sqlite3.Connection, pending: PendingConfirmation) -> None:
    """Insert one held request; a taken id or an unknown device is an invariant broken."""
    params = (
        pending.id,
        pending.device_id,
...
def _settle(
    ...
    """Re-read the confirmation, apply the one settlement rule and write the result."""
    with transaction(connection):
        row = connection.execute(_SELECT_SQL, (pending_id,)).fetchone()
        if row is None:
            raise PendingNotFoundError(pending_id)
        current = PendingConfirmation.model_validate_json(row["body"])
        settled = settle_pending(current, expected, settlement)
        connection.execute(
