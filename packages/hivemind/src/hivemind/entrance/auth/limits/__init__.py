...
``rate`` is ``RateLimiter``, token buckets per device (``rate_limit_per_device``) and per address
(``rate_limit_per_address``); ``denials`` is ``DenialCounter``, which locks a device whose
...
Public API:
    - RateLimiter, MAX_TRACKED_KEYS: token buckets per device and per address (rate).
...
from hivemind.entrance.auth.limits.rate import MAX_TRACKED_KEYS, RateLimiter
