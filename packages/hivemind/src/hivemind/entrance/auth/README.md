...
bound to its session (ADR-0033). Each module here is pure or a thin wrapper; sessions, login
challenges and step-up (roadmap 10.5e) are built on top of them.
...
`registration_options`, `authentication_options`, `verify_registration`, `verify_authentication`;
`SoftPasskey`; and from `canonical`: the five string builders, their tags, `b64url_encode`,
