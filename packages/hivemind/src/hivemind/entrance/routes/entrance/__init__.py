...
to join, approval and denial on loopback, and the steward route) and ``held`` (the requests held for
a person's confirmation). Every row declares its listeners: the loopback-only ones are never
mounted on the remote listener.

Fits into the Hive:
    Layer 7 (edges: HTTP, terminal, dashboard), inside ``hivemind.entrance.routes``. Its three row
...
__all__ = ["DOOR_ROUTES", "ENROLMENT_ROUTES", "HELD_ROUTES"]
