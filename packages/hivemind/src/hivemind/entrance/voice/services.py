...
See Also:
    - hivemind.llm.transcription for Ears, what the door hears through, and
      hivemind.llm.fanner.transcription for FannerTranscriptionGate, which meters it.
...
class VoiceServices:
    ...
    Attributes:
        ears: The TRANSCRIBER slot's bound chain and the gate it is heard through: at the
            composition root, the Fanner's metered ``FannerTranscriptionGate``, so every clip
            is one ``llm.call`` on the Hive's trail.
