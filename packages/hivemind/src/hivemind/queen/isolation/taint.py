...
clears it (`hivemind.memory.taint.clear_taint`), a lift included.
...
Fits into the Hive:
    ...
    `hivemind.queen.attach`. Calls into `hivemind.common.logging`, `hivemind.memory`
    (MemoryContext, the taint setter), `hivemind.pheromone` (TrailQuery), the sub-package's own
    order, pause and record modules, and waggle only; `QueenDeps` only for its type.
...
async def taint_cell(
    ...
    Returns:
        How many items of the Hive's own tables were newly labelled.
    """
    order = await _order(deps, link, reason, cause_event_id, suspect_at)
    scope = TaintScope(
        authors=frozenset(order.authors), task_ids=frozenset(order.task_ids), since=suspect_at
    )
    stamp = TaintStamp(source=TaintSource.ISOLATION, reason=reason, cause_event_id=cause_event_id)
...
async def _order(
    ...
    bees = (link.warden_id, *(await _cell_bees(deps, tasks))[: MAX_TAINT_AUTHORS - 1])
...
async def _cell_bees(deps: QueenDeps, tasks: tuple[TaskId, ...]) -> tuple[str, ...]:
    """Return the sub-bees a Warden spawned for one of `tasks`, newest first."""
    if not tasks:
        return ()
