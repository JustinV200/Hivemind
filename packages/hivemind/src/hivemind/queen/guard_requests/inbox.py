...
(`hivemind.queen.ticks.human.chat.human_items`): every request her table still holds undecided
becomes one item, aged from when her door filed it, so a request that waited out a restart keeps its
age. The item carries the report itself as its payload; the decision
