...
Fits into the Hive:
    ...
    sub-package. Called by `hivemind.queen.dispatcher.ready.assign.send_grant_and_assign` on
    every fresh grant (a first dispatch, a retry, a resume). Calls into `hivemind.brood_chamber`
    (Task), `hivemind.forage` (ForageGrant), `hivemind.guard`, `hivemind.queen.authority` and
