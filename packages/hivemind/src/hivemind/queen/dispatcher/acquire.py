...
async def _retry_once(
    ...
    if isinstance(placement, ProvisionVirtual):
        ...
        inventory = await build_inventory(deps, wardens, task=task, virtual_backends=zeroed)
    else:
        excluded = frozenset({placement.cell_id})
        inventory = await build_inventory(deps, wardens, task=task, exclude_dormant=excluded)
    decided = decide(
