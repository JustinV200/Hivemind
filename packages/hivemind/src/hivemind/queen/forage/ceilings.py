...
`waggle.messages.forage.CeilingsSet` to the Warden over its `WardenLink` (the same
`link.transport.send(wrap(...))` shape `hivemind.queen.dispatcher._send_grant_and_assign` already
uses for `GrantIssued`), and record `forage.ceilings_set` -- in that order, so the trail always
shows a ceilings change that has already reached its Warden.
