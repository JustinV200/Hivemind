...
Revoking a device at the Hive Entrance (roadmap 10.5d, ADR-0041) ends its sessions at once, but the
work it asked for lives in the Queen's own tables, so the Entrance asks her to withdraw it rather
than writing them itself. `refuse_unplanned` refuses every goal request the device submitted that
has not been planned yet (RECEIVED, AWAITING_CONFIRMATION or PLANNING), so a revoked device's work
never starts; a request whose plan is in flight is refused too, and the plan finishing later stops
the goal it produced (`hivemind.queen.ticks.human.intake`), while a goal its plan already persisted
(the plan landed, or a crash left it) is stopped here at once. `stop_goal` ends a planned goal: a
task not yet placed is cancelled in the Brood Chamber, and a placed one is first stopped on its
Warden with a `TaskCancel` (the Warden relays it to the sub-bee running the task,
...
`task.cancelled` herself, and whatever the Warden reports about that task afterwards is a stale echo
her autopilot only records (`hivemind.queen.autopilot.table`). A placed task whose Warden is not
attached cannot be stopped, so it is left running and the goal is reported unfinished.
