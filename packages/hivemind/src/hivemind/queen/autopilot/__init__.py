...
outright -- exactly what "within headroom, no awake episode" must not be. Nothing under this
...
Fits into the Hive:
    ...
    `hivemind.queen.ticks.forage` for a ForageRequest specifically.
...
Key invariants:
    ...
    - `decide` is pure: the same `(item, task, attempts, policy, limit)` always returns the same
      `QueenAction`; `decide_forage_request` is pure the same way over its own single argument.
...
See Also:
    ...
    - .claude/roadmap.md phase 3 step 3.20 for the work that first populated this package; phase 4
      step 4.7 for the ForageRequest rule.
