...
step 10.5: her reply is read by a person), a Guard request gets `Effort.HIGH` (roadmap step
10.6a: isolating a Cell or quarantining a bee costs a task if wrong), and everything else -- a
Heartbeat, a routine task report, an unrecognised kind -- gets `Effort.LOW`. A table, not a chain
of `if`s, so a new event class is one row, not a new branch.
...
_EFFORT_BY_KIND: Mapping[InboxKind, Effort] = {
    ...
    InboxKind.GUARD_REQUEST: Effort.HIGH,  # A wrong call isolates a Cell or leaves one open.
}
...
def effort_for(kind: InboxKind) -> Effort:
    ...
    Returns:
        Effort.HIGH for an Alarm or a Guard request, Effort.MEDIUM for a Question or a human's
        chat message, Effort.LOW for everything else.
    """
