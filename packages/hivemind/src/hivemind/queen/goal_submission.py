...
(codingrules section 12); the Brood Chamber cuts the tasks' own transitions itself. Roadmap step
7.9 puts the planner's Honey consultation in `plan_goal_graph`: before planning, the Queen asks
the Honey Store (the Hive's ripened, searchable knowledge) about the goal, reading every scope but
nothing above the goal's own clearance, scans every hit (roadmap step 10.6b) and hands them to the
planner on `PlanBrief.honey`; once the goal has an id she records `queen.honey_consulted` with
stage `plan`.

Fits into the Hive:
    Layer 6 (the kernel; the only global view; divides Forage), inside the queen package. Called
    by `hivemind.queen.queen.Queen.submit_goal` (`submit_goal`) and
    `hivemind.queen.ticks.human.intake` (`plan_goal_graph`). Calls into
    `hivemind.brood_chamber.task` (GoalRequestId), `hivemind.cell`, `hivemind.forage.slots`
    (ModelSlot), `hivemind.guard` (the location asks' refusals), `hivemind.queen.authority`,
    `hivemind.queen.deps`, `hivemind.queen.dispatcher` (dispatch_ready, consult_for_plan,
    screen_for_plan, record_consulted), `hivemind.queen.planner` (PlanBrief, plan_goal,
    location), `hivemind.pheromone` (segments_of), `hivemind.queen.trail` (record_event) and
    waggle only.
...
async def plan_goal_graph(
    ...
    consultation = await consult_for_plan(deps, goal, terms.clearance)
    brief = await _brief(deps, wardens, goal, terms, consultation)
...
async def _brief(
    ...
    """Build the planner's brief: the goal, its terms, the fleet and the Honey it may read."""
    # Each Honey hit is scanned before the planner reads it (roadmap 10.6b).
    honey = (
        await screen_for_plan(deps, consultation.hits, terms.comb_shield)
        if consultation is not None
        else ()
    )
    return PlanBrief(
