...
plan-and-persist half with the request's budget, tier, origin, device, ceiling and id, then PLANNED
with the goal id, or REFUSED with the reason. A spoken goal (or one held for a step-up) waits in
AWAITING_CONFIRMATION until the human confirms it. On a restart, a PLANNING row whose goal already
exists in the Brood Chamber (found by the request id every task carries) becomes PLANNED, and one
whose goal does not is planned again, so a crash after the `202` loses nothing and never plans a
goal twice. Planning is never a task the Entrance owns, so reducing the Entrance cannot kill it.
