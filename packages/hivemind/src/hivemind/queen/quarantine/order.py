...
async def order_quarantine(
    ...
    Returns:
        ...
        task, the task is unknown or not placed on an attached Warden, or its link closed before
        the order could go (nothing was sent, nothing recorded).
    """
    link = await _warden_of(deps, wardens, lever)
    if link is None or lever.task_id is None:
        return False
    # Guarded: a link that closed under the send is logged and reported, never raised.
    order = wrap(to_intervene(lever, alarm_id=alarm_id), link.hop, clock=deps.clock)
    if not await link.send(order):
        return False
    await record_event(
        deps,
        "queen.decided",
