    ...
    *,
    ...
    Args:
        brief: The goal, its clearance ceiling, the Cells the Hive can place work on (rendered
            by `describe_fleet` into hot state; None omits the section) and its `scratch_root`,
            passed on as a validation context so `PlannedTask`'s own leaves-vs-scratch rule can
            fire inside the ladder below.
