...
ADR-0036 requires: a `fallback` link is only ever built when it serves the *same* model id as the
...
class BoundEmbedder:
    ...
    model: str  # The model id every vector from `provider` is tagged with (ADR-0036).
