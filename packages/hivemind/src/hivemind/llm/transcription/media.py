...
Fits into the Hive:
    ...
    `hivemind.llm.transcription.models` (an `AudioClip` carries one), by the Entrance's voice
    route (the upload's own label) and by the openai_compat transcription adapter (the upload's
    filename). Every adapter shipped decodes all five: faster-whisper through its own decoder,
    and a Whisper-style server by the filename's extension. Imports only this package's errors.

Key invariants:
    - A member's value is the canonical media type sent on the wire for that format, and a
      member compares equal to it (a `StrEnum`), as the WAV-only `Literal` it replaced did.
