...
- **`openai_compat/`** (roadmap step 3.7): speaks the OpenAI-compatible chat-completions wire to
  a local or self-hosted server (Ollama, vLLM, llama.cpp's built-in server, LM Studio), and
  (roadmap step 6.5a's subset for 10.5f) the audio transcription wire, as
  `OpenAICompatTranscription`, for the `TRANSCRIBER` slot. See its own `README.md` for capability
  declaration and error mapping.
- **`whisper/`** (roadmap step 6.5a proper, not yet built): faster-whisper in process, an
  optional extra; see `hivemind.llm`'s README for what remains.
...
(`OpenAICompatProvider`/`OpenAICompatConfig` today, and the transcription pair
`OpenAICompatTranscription`/`OpenAICompatTranscriptionConfig`) -- never a sub-package's internal wire-mapping
