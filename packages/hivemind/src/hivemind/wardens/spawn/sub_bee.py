...
grant's allowed_bindings" search), its last `HandoffRef` (for a RETRY's `resume_from`), and the
`asyncio.Task` running its `hivemind.workers.runtime.WorkerRuntime` inside the Warden's own
...
Fits into the Hive:
    ...
    sub-bee sends. Calls into `hivemind.workers` (WorkerState) and waggle only.
...
Key invariants:
    ...
    - `link` is the Warden's own end of the `waggle.transport.memory.MemoryTransport` pair; the
      ...
      terminal state (or is killed) and the Warden is done with it.
...
See Also:
    ...
    - hivemind.wardens.spawn.spawn for spawn_sub_bee, this record's one builder.
"""
...
class SubBee:
    ...
    Attributes:
        ...
        runtime_task: The asyncio.Task running this sub-bee's WorkerRuntime, owned by the Warden's
            own TaskGroup; awaited or cancelled by the Warden, never dropped.
