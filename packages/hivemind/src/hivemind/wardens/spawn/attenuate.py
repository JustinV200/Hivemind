...
root), the task's needs (its Exoskeleton need and network scopes, `TaskAssign.exoskeleton` and
`.network_scopes`, both since Waggle minor 6, ADR-0031), the goal's set (`TaskAssign.capabilities`,
Waggle minor 8; None when the goal carries no ceiling) and the declared write roots (roadmap step
5.0e: the manifest's `keep_root` and each declared leaving's root).
...
def _task_needs(assignment: TaskAssign) -> TaskNeeds:
    """Build the TaskNeeds `worker_capabilities` reads: tempo, Exoskeleton, every readable scope."""
    scopes = tuple(scope for scope in assignment.network_scopes if _is_net_scope(scope))
    # The Exoskeleton need travels too (ADR-0031): dropping it would spawn a Worker whose task
    # asked for a browser or desktop without the `exoskeleton` capabilities to drive one.
    return TaskNeeds.from_wire(Tempo.from_wire(assignment.tempo), assignment.exoskeleton, scopes)
