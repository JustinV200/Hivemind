...
and access level as the context; a refusal is a `guard.denied` row and a WATCH, exactly like a
lease the source itself refused.
...
async def open_lease(warden: Warden) -> None:
    ...
    warden._ceiling = warden_set(warden._deps.guard, lease.access_level, lease.scratch_root)
