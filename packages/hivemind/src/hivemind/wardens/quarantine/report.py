...
async def _send(warden: Warden, message: WaggleMessage) -> None:
    ...
    deps = warden._deps
    # Guarded: a Queen link gone mid-quarantine is logged, never the end of this Warden's tick.
    await send_guarded(deps.queen_link, wrap(message, deps.hop, clock=deps.clock))
