...
A Task (`hivemind.brood_chamber.task.Task`) moves through a fixed set of states from submission to
a terminal outcome. This module is the state machine codingrules section 9 requires for every
machine in the Hive: one `Enum` (`TaskStatus`) plus one transition table (`TRANSITIONS`), each
allowed edge commented with who causes it, tested edge by edge (Appendix C, the "Task" row).
Nothing else in the Hive decides whether a transition is legal; `hivemind.brood_chamber.chamber`
(roadmap step 2.8) calls `assert_transition` before writing a new `Task.status`, and
`hivemind.brood_chamber.task.Task`'s own validators check the *consequences* of a status (whether
`outcome`, `warden_id` and so on are set) without re-deciding which transitions exist.

Fits into the Hive:
    Layer 2 (the Cell abstraction, state, memory, policy). Read by `hivemind.brood_chamber.chamber`
    (roadmap step 2.8) before every state change, and by `hivemind.brood_chamber.graph` (which
...
See Also:
    ...
    - hivemind.brood_chamber.task for Task, the model whose `status` field this machine governs.
"""
