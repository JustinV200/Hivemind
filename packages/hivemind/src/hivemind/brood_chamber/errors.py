...
that already exists, or asking either state machine (`task.state.TRANSITIONS`,
...
Question-specific class: the two machines (`task/state.py`, `questions.py`) never call each other,
...
because the id it carries is a Task's id when `task.state.assert_transition` raises it and a
...
Fits into the Hive:
    ...
    task.state`, `hivemind.brood_chamber.questions`, `hivemind.brood_chamber.task.model`
    (validators) and the store the later phase 2 steps add (`hivemind.brood_chamber.
    store.protocol`, `hivemind.brood_chamber.store.sqlite`, `hivemind.brood_chamber.store.memory`).
    Imported by every layer above that submits, reads or advances a task or a question.
...
See Also:
    ...
    - hivemind.brood_chamber.task.state for TRANSITIONS, the table InvalidTransitionError reports.
...
class InvalidTransitionError(ConflictError):
    ...
    Shared by `task.state.assert_transition` (Task) and `questions.assert_question_transition`
...
class InvalidGraphError(BroodChamberError):
    ...
    2.8) raises once a graph has already passed pydantic validation but still fails a check made
    outside it, so a caller of the Brood Chamber's public API catches one typed error either way.
    """
