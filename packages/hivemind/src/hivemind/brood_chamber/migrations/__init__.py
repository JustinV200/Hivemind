...
brood_chamber.sqlite.apply_brood_chamber_migrations` reads them through `importlib.resources.
files("hivemind.brood_chamber.migrations")`, which is exactly why this needs to be a real Python
...
Fits into the Hive:
    Layer 2 (the Cell abstraction, state, memory, policy). Read by hivemind.brood_chamber.sqlite;
...
See Also:
    ...
    - hivemind.brood_chamber.sqlite for apply_brood_chamber_migrations, the one caller.
