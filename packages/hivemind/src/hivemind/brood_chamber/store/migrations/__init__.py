"""Hold the Brood Chamber's numbered SQL migration series; carries no code of its own.

Every file directly under this directory matching `NNNN_name.sql` (`hivemind.common.migrations.
MIGRATION_FILE_PATTERN`) is one migration in the Brood Chamber's schema series. `hivemind.
brood_chamber.store.sqlite.apply_brood_chamber_migrations` reads them through `importlib.resources.
files("hivemind.brood_chamber.store.migrations")`, which is exactly why this needs to be a real
Python package (an `__init__.py`, however empty) rather than a bare directory: `importlib.resources`
addresses packages by dotted name, not by filesystem path, so the files ship inside the installed
...
Fits into the Hive:
    ...
    hivemind.brood_chamber.store.sqlite; nothing imports this module for a Python name.
...
See Also:
    ...
    - hivemind.brood_chamber.store.sqlite for apply_brood_chamber_migrations, the one caller.
