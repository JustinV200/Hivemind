...
without a SQLite file. It implements `hivemind.brood_chamber.store.protocol.TaskStore` exactly like
`hivemind.brood_chamber.store.sqlite.SqliteTaskStore` does, which is what the contract suite
...
Fits into the Hive:
    ...
    database file. Calls into hivemind.brood_chamber (store.protocol, task.model, questions,
    task.state, errors), hivemind.common.errors and hivemind.pheromone (`PheromoneTrail`,
    `TaskEvent`) only.
...
See Also:
    - hivemind.brood_chamber.store.protocol for the TaskStore protocol this class implements, and
      check_task_event, the guard every mutation calls first.
    - hivemind.brood_chamber.store.sqlite for the durable counterpart.
    - hivemind.pheromone.trail.memory for MemoryPheromoneTrail, the PheromoneTrail this store is
      built on in tests, and the equivalent "validate, record, then swap in" shape it follows.
"""
