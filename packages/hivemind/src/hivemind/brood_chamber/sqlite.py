...
(`hivemind.brood_chamber.migrations`), and `SqliteTaskStore`, the `hivemind.brood_chamber.store.
TaskStore` implementation built on them. Every mutation runs one transaction under `asyncio.
to_thread` that writes the task and/or question row(s) and calls `hivemind.pheromone.insert_event`
for the accompanying event on the same connection, so the state change and its trail event commit
together (codingrules section 12's "same transaction" rule, Appendix C rule 3) -- exactly the
pattern `hivemind.pheromone.sqlite.insert_event`'s own docstring names the Brood Chamber as the
first caller of.
...
Fits into the Hive:
    ...
    hivemind.common (connect, transaction, migrations, errors), hivemind.brood_chamber (store,
    task, questions, errors) and hivemind.pheromone (insert_event) only.
...
See Also:
    ...
    - hivemind.brood_chamber.store for the TaskStore protocol and check_task_event, the guard
      every mutation calls before writing.
    - hivemind.pheromone.sqlite for insert_event, the primitive this module calls inside its own
      transactions, and the SQLite adapter this module's shape mirrors.
"""
...
MIGRATIONS_PACKAGE = "hivemind.brood_chamber.migrations"
...
def apply_brood_chamber_migrations(connection: sqlite3.Connection, clock: Clock) -> tuple[int, ...]:
    """Apply every pending migration under `hivemind.brood_chamber.migrations`.
...
class SqliteTaskStore:
    ...
    def __init__(self, connection: sqlite3.Connection) -> None:
        ...
        # Serialises every method on this instance, matching hivemind.pheromone.sqlite.
    ...
    async def create(cls, connection: sqlite3.Connection, clock: Clock) -> SqliteTaskStore:
        ...
        if not has_pheromone_table:
            raise MigrationError(
                ...
                "connection; call hivemind.pheromone.sqlite.apply_pheromone_migrations (or "
