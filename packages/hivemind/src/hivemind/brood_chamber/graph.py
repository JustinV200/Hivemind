...
A task graph is a set of Tasks (`hivemind.brood_chamber.task.Task`) linked by
...
than fixed to `TaskId`, because `hivemind.brood_chamber.task.TaskGraphDraft` (a submission before
any `TaskId` has been minted) needs the same cycle check over its own string keys; that is also why
this module never imports `hivemind.brood_chamber.task` at the top level (see Key invariants). The
DFS is written iteratively with an explicit stack, never recursively: `MAX_GRAPH_TASKS`
(`hivemind.brood_chamber.task`) allows graphs up to 256 tasks, comfortably inside Python's default
recursion limit, but a pure function with no I/O has no good reason to depend on the caller's stack
depth at all, so the iterative form is used regardless.

Fits into the Hive:
    Layer 2 (the Cell abstraction, state, memory, policy). Called by
    `hivemind.brood_chamber.task.TaskGraphDraft` (`is_acyclic_edges`, over draft keys) and by
    `hivemind.brood_chamber.chamber` (roadmap step 2.8: `ready_tasks` for `next_ready`,
    `descendants` when cancelling a goal). Calls into `hivemind.brood_chamber.task_state` only, for
    `TaskStatus`.

Key invariants:
    - This module never imports `hivemind.brood_chamber.task` except under `TYPE_CHECKING`: `Task`
      is used only in type hints (`is_acyclic`, `ready_tasks`, `descendants` all take
      `Iterable[Task]` but only ever touch `.id`, `.status` and `.spec.depends_on` at runtime, so
      no import is needed to run). `hivemind.brood_chamber.task` imports this module for real (to
      call `is_acyclic_edges` from `TaskGraphDraft`'s validator); keeping the `Task` import
      type-checking-only here is what keeps that a one-way runtime dependency instead of a cycle.
...
See Also:
    ...
    - hivemind.brood_chamber.task for Task and TaskGraphDraft, the two callers of this module.
    - hivemind.brood_chamber.task_state for TaskStatus, read by ready_tasks.
"""
