...
(`hivemind.brood_chamber.task`) the Queen decomposes a goal into, and every `Question`
...
`check_task_event` is the one guard both implementations (`hivemind.brood_chamber.memory.
MemoryTaskStore`, `hivemind.brood_chamber.sqlite.SqliteTaskStore`) call before writing, so a bug
inside the Brood Chamber can never file an event under the wrong task's `subject_id` or the wrong
event family.

Fits into the Hive:
    Layer 2 (the Cell abstraction, state, memory, policy). Implemented by
    `hivemind.brood_chamber.memory` and `hivemind.brood_chamber.sqlite`; used by
    `hivemind.brood_chamber.chamber` (roadmap step 2.8), the only caller that decides *when* a
    transition happens. Calls into `hivemind.brood_chamber.task`, `hivemind.brood_chamber.
    questions`, `hivemind.brood_chamber.task_state`, `hivemind.common.errors` and
...
See Also:
    ...
    - hivemind.brood_chamber.memory and hivemind.brood_chamber.sqlite for the two implementations.
...
MAX_TASK_FILTER_LIMIT = 10_000  # Matches pheromone.trail.MAX_QUERY_LIMIT's page-size ceiling.
...
class TaskStore(Protocol):
    ...
    Implementations (`hivemind.brood_chamber.memory.MemoryTaskStore`,
    `hivemind.brood_chamber.sqlite.SqliteTaskStore`) must be safe to call concurrently. Every
