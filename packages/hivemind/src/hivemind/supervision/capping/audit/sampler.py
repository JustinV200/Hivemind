...
Codingrules section 8.12: "What cannot be gated is sampled. Tiers the table marks as ungated in real
time are audited after the fact by the judge; findings become Nectar and Alarms, and the Guard Bee
raises a tier's rate when its failure rate climbs." A tier whose `TierSpec.judge` is False never
sees `CheckKind.JUDGE` in its real-time ladder (`hivemind.supervision.capping.tiers.checks_for`);
this module is the fallback for exactly those tiers. Once a proposal has already reached a terminal
state (`VERIFIED` or `ROLLED_BACK`), `AuditSampler` decides -- deterministically by default, so a
test never needs retries -- whether this one is sampled at its tier's `TierSpec.audit_rate`, and
`audit_completed` runs the same `JudgeReviewer` the real-time `JUDGE` check would have used,
deposits the finding through a `FindingsSink` (Nectar, the Hive's raw intake before Honey ripens it
-- the Honey Store itself is phase 7, so this module only defines the seam and ships an in-memory
implementation), records a `capping.audited` trail event, and raises an `Alarm` of kind
`AUDIT_FAILED` on a `REJECT` verdict, through the same `record_alarm_event` path every other
supervisor uses. `AuditRates` is the read model the Guard Bee (phase 10, part of `hivemind.guard`,
the Hive's policy engine) will read to raise a tier's `audit_rate` when its failure rate climbs;
this module only counts, it does not decide. The raise itself (roadmap step 10.6) lives in
`hivemind.supervision.capping.audit.raises`. `review_applied` is the one review that is not sampled:
an irreversible GUI proposal is judged right after it is applied, with the flight recorder's
evidence, and a judge that cannot answer counts as a rejection (ADR-0032); its Alarm is the
proposer's to raise, on the path that ends the attempt.
...
Key invariants:
    ...
    - audit_completed never raises for an ordinary REJECT verdict: that path deposits a finding and
      ...
      wired, as a Virtual Cell's in-Cell Warden is today; found on a real Docker run,
      2026-09-24). It raises
...
async def audit_completed(
    ...
    Args:
        ...
        evidence: What the action did, when something recorded it (a GUI proposal's flight
            recording, ADR-0032); the judge sees it beside the action.
    ...
    try:
        ...
        # one, found on a real Docker Virtual Cell run, 2026-09-24, and again by roadmap step
        # 10.6's raise). An audit is a sample taken after the proposal already landed, so a missed
        # sample changes nothing about the work; it is recorded as inconclusive and the Worker
        # carries on. Found by a scratch_write audit (2 % sampling) against a scripted provider
        # with no judge answer: the error propagated out of the gate and crashed the Drone
        # mid-task (2026-09-22).
...
async def review_applied(
    deps: AuditDeps, proposal: Proposal, evidence: JudgeEvidence | None
) -> JudgeVerdict:
    """Review an applied proposal now, every time; a judge that cannot answer counts as a REJECT.
    ...
    Args:
        ...
        proposal: The applied, VERIFIED proposal.
        evidence: What the action did, or None when nothing was recorded.

    Returns:
        The judge's verdict, or a REJECT that says the judge could not answer.
...
async def _review(
    deps: AuditDeps, proposal: Proposal, evidence: JudgeEvidence | None = None
) -> JudgeVerdict:
