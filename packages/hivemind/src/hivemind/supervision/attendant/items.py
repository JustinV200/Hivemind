...
10.6a, ADR-0043) a Guard Bee's request that the Queen act on one Cell or one bee. `InboxKind` is
...
Key invariants:
    ...
    - A GUARD_REQUEST item carries no severity, no task and no latency budget (a validator does
      hold this one): its score is its fixed kind weight and its age alone (ADR-0043), so two
...
class InboxKind(Enum):
    ...
    GUARD_REQUEST = "GUARD_REQUEST"  # A Guard Bee's request (ADR-0043); the Queen's inbox only.
...
class InboxItem(BaseModel):
    ...
    def _guard_request_scores_on_kind_and_age(self) -> InboxItem:
        ...
        # ADR-0043: a Guard request's weight is fixed; a task link, an urgency or a severity
