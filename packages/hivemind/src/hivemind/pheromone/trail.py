...
Protocol, so hivemind.pheromone.memory (an in-process fake) and hivemind.pheromone.sqlite (the
durable store) are interchangeable to everything above them. A segment is one node's (one Queen or
Warden process, ADR-0005) slice of the trail: a Warden that is offline keeps recording to its own
segment and merges it into the Queen's trail on reconnection, which is why merge_segment must be
safe to call twice with the same segment.

Fits into the Hive:
    Layer 1 (foundational services; capacity as data). Implemented by hivemind.pheromone.memory
    and hivemind.pheromone.sqlite; used by every layer above Layer 1 that records or reads trail
    events, and by hivemind.pheromone.tail and hivemind.pheromone.retention. Calls into
...
See Also:
    ...
    - hivemind.pheromone.memory and hivemind.pheromone.sqlite for the two implementations.
...
class PheromoneTrail(Protocol):
    ...
    Implementations (`hivemind.pheromone.memory.MemoryPheromoneTrail`,
    `hivemind.pheromone.sqlite.SqlitePheromoneTrail`) must be safe to call concurrently: the Queen
