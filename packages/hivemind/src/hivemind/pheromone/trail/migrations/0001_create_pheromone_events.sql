...
-- lexically the same as chronologically (ADR-0006). hivemind.pheromone.trail.sqlite issues only
-- INSERT and INSERT OR IGNORE statements against this table: it never rewrites or removes a row
-- once written. The one place in hivemind.pheromone that ever removes a row is
-- hivemind.pheromone.retention, the Night Veil boundary (ADR-0007, codingrules section 12), and
-- only at teardown.
...
-- triple (hivemind.pheromone.trail.protocol.TRAIL_ORDER_KEY), so it is also the index that
-- carries them.
