...
its own tests. It implements `hivemind.pheromone.trail.protocol.PheromoneTrail` exactly like
`hivemind.pheromone.trail.sqlite.SqlitePheromoneTrail` does, which is what the contract suite
...
Fits into the Hive:
    ...
    (errors, events, trail.protocol) and waggle only.
...
Key invariants:
    ...
    - Every read (query, export_segment) returns events sorted by TRAIL_ORDER_KEY, matching
      hivemind.pheromone.trail.sqlite exactly.
...
See Also:
    - hivemind.pheromone.trail.protocol for the PheromoneTrail protocol this class implements.
    - hivemind.pheromone.trail.sqlite for the durable counterpart.
...
from hivemind.pheromone.trail.protocol import TRAIL_ORDER_KEY, TrailQuery, TrailSegment
from waggle.clock import Clock
from waggle.ids import NodeId

__all__ = ["MemoryPheromoneTrail"]

# Sorts a list of events the same way hivemind.pheromone.trail.sqlite's `ORDER BY at, node_id,
# rowid` does; built once from TRAIL_ORDER_KEY so the two implementations can never drift apart.
