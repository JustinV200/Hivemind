...
(`hivemind.pheromone.trail.migrations`), the one parametrised statement that writes a row
...
transaction" rule), and `SqlitePheromoneTrail`, the `hivemind.pheromone.trail.protocol.
PheromoneTrail` implementation built on both. Every statement in this file is an `INSERT` or an
`INSERT OR IGNORE`: it never rewrites or removes a row it has already written, so the append-only
guarantee is provable by reading this one file rather than by trusting a convention (decision 7 in
...
Fits into the Hive:
    ...
    (connect, transaction, migrations) and hivemind.pheromone.events/trail.protocol/errors.
...
See Also:
    ...
    - hivemind.pheromone.trail.protocol for the PheromoneTrail protocol and TrailQuery/TrailSegment.
...
MIGRATIONS_PACKAGE = "hivemind.pheromone.trail.migrations"
...
# Mirrors hivemind.pheromone.trail.protocol.TRAIL_ORDER_KEY = ("at", "node_id") plus rowid, the
# insertion order, as the tiebreaker (see the comment on TRAIL_ORDER_KEY for why the id is not
# it); kept in sync by test_sqlite_order_by_matches_trail_order_key.
...
def apply_pheromone_migrations(connection: sqlite3.Connection, clock: Clock) -> tuple[int, ...]:
    """Apply every pending migration under `hivemind.pheromone.trail.migrations`.
