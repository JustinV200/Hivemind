...
`item_cap_chars` (`text`), and an optional full payload for the two kinds whose content lives here
and nowhere else yet (`payload`, for `TRANSCRIPT` and `TOOL_RESULT`: the Honey Store does not exist
until phase 7, so a deposited transcript or an oversized tool result is stored here in full, and
whatever holds the entry's own id -- hot state, a Handoff -- carries only that id as its
"reference", never the payload text itself).
...
Key invariants:
    ...
    - `text` and `payload` are never both set by this dispatch's own writers
      (hivemind.memory.bee_bread.deposit): an index-only entry (TASK_HISTORY, TRAIL_EVENT, HANDOFF,
      NOTE) carries `text` and no `payload`; a full-content entry (TRANSCRIPT, TOOL_RESULT) carries
      `payload` and no `text`. Nothing in this model enforces that split; it is a convention its one
      writer module follows.
...
MAX_REF_IDS = 8  # An entry indexes a handful of records at most; more means several entries.
...
class BeeBreadEntry(BaseModel):
    ...
    payload: str | None = Field(
        ...
        description="The full content for a TRANSCRIPT or TOOL_RESULT entry; unset otherwise.",
    )
