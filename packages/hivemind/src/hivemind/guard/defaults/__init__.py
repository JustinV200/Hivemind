"""Hold the Guard policy a Hive falls back to; carries no code.
...
supply. It is read through `importlib.resources.files("hivemind.guard.defaults")`, which is why
this is a real Python package (an `__init__.py`, however empty) rather than a bare directory:
`importlib.resources` addresses packages by dotted name, so the file ships inside the installed
distribution and resolves the same from a checkout, from a wheel and from a Hive directory that
is neither.

Fits into the Hive:
    Layer 2 (the Cell abstraction, state, memory, policy). Read by
    `hivemind.guard.policy.defaults.load_guard_policy` when it is given no policy file; nothing
...
See Also:
    - hivemind.guard.policy.defaults for load_guard_policy, this package's one reader.
    - hivemind.supervision.defaults for the same pattern applied to the escalation policy.

Public API: none; this package holds a data file (`policy.toml`), not importable names.
"""
