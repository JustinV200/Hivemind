...
AUTHORISED_AT: tuple[tuple[str, P], ...] = (
    ...
    # Phase 6 (the Exoskeleton): peripherals attached for one task, the operator's own display
    # only for a set holding exoskeleton:real_display.
    ("cell.exoskeleton_attached", P.EXOSKELETON_REAL_DISPLAY),
    # Phase 7 (the Honey Store): every read and write of Honey at a clearance.
    ("honey.nectar_received", P.HONEY_ACCESS),  # A deposit written into intake.
    ("honey.ripened", P.HONEY_ACCESS),  # The House Bee writing Honey rows (honey:write).
    ("honey.label_lowered", P.HONEY_ACCESS),  # Readable more widely: a judge's or the human's.
    ("honey.queried", P.HONEY_ACCESS),  # A read, up to the reader's clearance and scopes.
)
...
NOT_ACTIONS: tuple[tuple[str, str], ...] = (
    ...
    ("cell.exoskeleton_detached", _LIFECYCLE),  # The end of an attach: its processes stopped.
    ("honey.nectar_deduplicated", _OUTCOME),  # A deposit met a stored one; a label only rose.
    ("honey.nectar_rejected", _REFUSAL),
    ("honey.ripen_failed", _OUTCOME),  # The Nectar stays pending for the next pass.
    ("honey.reembedded", _LIFECYCLE),  # Ripened rows gaining the current model's vectors.
    ("honey.label_raised", _NARROWING),
...
PENDING_POINTS: Mapping[P, str] = MappingProxyType(
    {
        # Phase 6 landed: attach already plans the operator's own display only for a set holding
        # `exoskeleton:real_display`; what is left is refusing through this point, on the trail.
        P.EXOSKELETON_REAL_DISPLAY: "a phase 6 follow-up (attach checks the set, not the Guard)",
        # Phase 7 landed: the Honey Store reads `honey:read` scopes and clearances off the reader's
        # set itself; what is left is refusing through this point, on the trail.
        P.HONEY_ACCESS: "a phase 7 follow-up (the store checks the set, not the Guard)",
