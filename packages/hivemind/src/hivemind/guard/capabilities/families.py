...
Worker's tools to an enrolled device's Entrance routes; `ScopeKind` names the seven ways a scope is
written and matched (`net` has a kind of its own, `HOST`, because a glob or a prefix over host names
and addresses cannot express "a subdomain of" or "an address inside this network");
`FAMILIES_BY_KIND` gives each family exactly one kind, laid out like the ADR's own table so the two
can be read side by side; and the enumerated and ordered kinds carry their closed value lists, taken
from the enums they mirror (`ModelSlot`, `CombShieldLevel`, `MaskTactic`, `HoneyClearance`) or, for
`exoskeleton`, from `EXOSKELETON_SCOPES` (ADR-0031's four peripherals, so a misspelt grant is
refused rather than silently granting nothing). `PARSE_ORDER` is the longest-first order a string is
read in, so `tool:request` and `tool:scope:cell` are their own families rather than tools named
`request` or `scope:cell`, and `observe:honey:hive` is never read as `observe`.
...
Key invariants:
    ...
    - Enumerated and ordered values are the lowercase member names of the enum they mirror, so a
      ...
      rank order, lowest first. `exoskeleton`, which mirrors no enum, reads `EXOSKELETON_SCOPES`.
...
__all__ = ["EXOSKELETON_SCOPES", "FAMILIES_BY_KIND", "PARSE_ORDER", "CapabilityFamily", "ScopeKind"]

# The Exoskeleton peripherals an `exoskeleton` capability may name (ADR-0031): a display the lease
# starts, the display already running on the Cell (the operator's own screen), audio and the
# browser; "*" covers all four, and nothing else parses.
...
class CapabilityFamily(Enum):
    ...
    EXOSKELETON = "exoskeleton"  # Driving one Exoskeleton peripheral on a Cell (ADR-0031).
        ...
        ScopeKind.GLOB: frozenset(
            {
                ...
                # A Honey Store scope glob (roadmap step 7.10): the store filters rows with the
                # same glob in SQL, so the two readings of one grant can never disagree.
        ...
        ScopeKind.ENUMERATED: frozenset(
            {
                ...
                CapabilityFamily.CELL_COMB_SHIELD,
...
_SCOPE_VALUES: Mapping[CapabilityFamily, tuple[str, ...]] = MappingProxyType(
    {
        ...
        CapabilityFamily.EXOSKELETON: tuple(sorted(EXOSKELETON_SCOPES)),
