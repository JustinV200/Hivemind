...
roadmap step 10.1 added the rest of the families; every name that module exported (phase 6's
`EXOSKELETON_SCOPES` and phase 7's `HONEY_READ` family included) is still importable from here and
from `hivemind.guard`.
...
Public API:
    - CapabilityFamily, ScopeKind, FAMILIES_BY_KIND, PARSE_ORDER, EXOSKELETON_SCOPES: the family
      table, and the four peripherals an `exoskeleton` scope may name (families).
