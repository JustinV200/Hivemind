...
ceiling"), only when that set allows it too. The final call is `CapabilitySet.attenuate`, which
either returns the computed slice unchanged or raises `CapabilityWideningError`; calling it here
is deliberately defensive (the slice is already filtered down to what the Warden allows before
that call), so a raise from it can only mean this function's own filtering has a bug, proving the
invariant rather than relying on it. Roadmap step 7.8's two Honey Store tools (`tool:recall`,
`tool:remember`) are named in the Drone's role default beside `tool:*`, so they survive a Warden
ceiling that one day lists tools by name instead of `tool:*`
(`hivemind.workers.tools.registry.build_registry` offers each only when it is allowed).
