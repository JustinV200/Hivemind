...
Fits into the Hive:
    ...
    import hivemind.llm), `hivemind.memory`, `hivemind.supervision.alarm`, this package's own
    `honey` and `reports`, and waggle only; the `WorkerRuntime` type it is built with is imported
    under `TYPE_CHECKING` only, so no runtime import cycle exists between this module and
