...
blocking-question channel), `honey` (the role's own Honey Store channel over that mailbox, and
each checkpoint's Handoff deposit, roadmap step 7.8), `reporter` (this Worker's own WorkerState
and every outgoing message) and `attempt` (starting, cancelling and interpreting one role
attempt), each under codingrules 5.1's size limits; this face re-exports only what a caller
outside this package needs (codingrules 5.2).
...
Key invariants:
    - Whatever is not re-exported here (`Mailbox`, `MailboxHoneyChannel`, `Reporter`,
      `AttemptManager`, `WorkerRuntime`'s own private helpers) is private to this package
      (codingrules 5.4).
