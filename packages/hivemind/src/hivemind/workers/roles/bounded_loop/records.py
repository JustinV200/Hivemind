...
Every side-effecting tool a bounded-loop role offers (`write_file`, `run_command`, `http_request`)
goes through the Capping gate and reports back through `hivemind.workers.tools.proposals.describe`,
whose own rendering always starts with a fixed `state=<VERIFIED|REJECTED|ROLLED_BACK>` token; every
other built-in tool (`read_file`, `ask`, the registry itself for an unknown tool or a schema
violation) reports a failure through one of a small, fixed set of literal templates.
`classify_error` reads exactly those two vocabularies -- never a model's own free-form text -- to
decide whether one call's result was a failure, and `target_for` names the one path/command/url a
...
Key invariants:
    ...
    - `is_lasting` holds for every call to a tool in `SIDE_EFFECT_TOOLS` and for a GUI action
      flagged irreversible; a Handoff's `do_not_redo` (built in `.fields`) names nothing else.
...
See Also:
    ...
    - hivemind.workers.roles.bounded_loop.executor for LoopExecutor, this module's caller.
    - hivemind.workers.roles.drone.outcome.records for the re-export that keeps the Drone's own
...
# looking at the call, since nothing else a bounded-loop role offers changes anything outside the
# model's own next turn.
...
class ToolCallRecord:
    ...
    (`hivemind.workers.roles.bounded_loop.executor.MAX_RECORDED_CALLS`) is read exactly once, at
