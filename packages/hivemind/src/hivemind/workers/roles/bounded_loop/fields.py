...
Every function here answers one Handoff field from data the attempt already has --
`hivemind.workers.roles.bounded_loop.records.ToolCallRecord`s, the one call refused for a
checkpoint, this attempt's own pins, and the assignment's own acceptance criteria -- never from a
second read of a conversation (codingrules section 8.8). Moved out of `hivemind.workers.roles.
drone.outcome.fields` unchanged (roadmap step 6.9): nothing here was ever Drone-specific.

Fits into the Hive:
    Layer 4 (roles that do the work), inside `hivemind.workers.roles.bounded_loop`. Called by
    `hivemind.workers.roles.bounded_loop.outcome.build_handoff_outcome`, this module's one caller.
    Calls into `hivemind.cell`, `hivemind.llm`, `hivemind.memory`, `hivemind.workers.context`,
    this package's `.records` and waggle only.

Key invariants:
    - Every derived list is capped to `MAX_DERIVED_LIST_ITEMS`, well under Handoff's own
      `MAX_LIST_ITEMS` (64, `hivemind.memory.handoff`): a bounded tool loop never comes close, but
      the cap holds regardless of a future round-cap change.
...
See Also:
    ...
    - hivemind.workers.roles.bounded_loop.records for ToolCallRecord, classify_error's own result,
      and target_for.
    - hivemind.workers.roles.drone.outcome.fields for the re-export that keeps the Drone's own
