"""Define build_claimed_outcome and build_handoff_outcome: the two ways one attempt ends.
...
`hivemind.workers.roles.drone.outcome.fields`.
...
Fits into the Hive:
    ...
    `hivemind.workers.roles.drone.outcome.fields`,
    `hivemind.workers.roles.drone.outcome.records` and waggle only.
...
Key invariants:
    ...
    - A Drone never marks its own work SUCCEEDED (codingrules section 8.7): `build_claimed_outcome`
...
See Also:
    ...
    - hivemind.workers.roles.drone.outcome.fields for every Handoff field this module fills from
...
from hivemind.workers.roles.drone.outcome import fields
...
async def build_handoff_outcome(
    ctx: WorkerContext, assignment: TaskAssign, executor: _RecordingExecutor
) -> WorkerOutcome:
    """Build the WorkerOutcome for a Drone attempt that stopped early to hand off.

    Defect 1 this dispatch fixes: every guidance field below used to be built as an empty tuple
    regardless of what the attempt actually did; each is now derived from `executor.records` (and,
    for `pinned_facts`/`next_steps`, from this attempt's own pins and the assignment's own
    acceptance criteria) by `hivemind.workers.roles.drone.outcome.fields`.
    ...
    Args:
        ...
        executor: This attempt's own `_RecordingExecutor`; `records` is every tool call it made
            before the handoff, `pending_call` the one it refused to run, if any (becomes the
            Handoff's one `open_threads` entry).
