"""Move an approved tool into the CombRegistry: the promotion package.

It moves a tool out of quarantine once it is approved for use, at hive scope (usable anywhere)
or Cell scope (usable only on one Cell).

Fits into the Hive:
    Layer 3 (sources of Cells, and capabilities handed down), inside the royal_jelly package.
    Handles moving an approved tool out of quarantine into the CombRegistry. Called by
