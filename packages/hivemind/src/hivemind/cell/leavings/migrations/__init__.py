"""Hold the Leavings ledger's numbered SQL migration series; carries no code of its own.

Every file directly under this directory matching `NNNN_name.sql` (`hivemind.common.migrations.
MIGRATION_FILE_PATTERN`) is one migration in the Leavings ledger's schema series. `hivemind.cell.
leavings.store_sqlite.apply_leavings_migrations` reads them through `importlib.resources.
files("hivemind.cell.leavings.migrations")`, which is exactly why this needs to be a real Python
package (an `__init__.py`, however empty) rather than a bare directory: `importlib.resources`
...
Fits into the Hive:
    ...
    hivemind.cell.leavings.store_sqlite; nothing imports this module for a Python name.
...
See Also:
    ...
    - hivemind.cell.leavings.store_sqlite for apply_leavings_migrations, the one caller.
