"""Define the Exoskeleton's vision capability: the compound_eye package.

A protocol and backends see a Cell's screen.

Fits into the Hive:
    Layer 3 (sources of Cells, and capabilities handed down), inside the exoskeleton package.
    Handles the vision capability: seeing a Cell's screen. Called by exoskeleton's public API on
    behalf of whatever calls exoskeleton itself; calls into sibling packages at Layer 3 or
    below, never back up into exoskeleton's other sub-packages directly.
