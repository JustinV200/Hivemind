...
def _voice(options: RigOptions, deps: QueenDeps) -> VoiceServices | None:
    ...
    return VoiceServices(
        ears=_ears(options.transcriber),
...
def _ears(transcriber: TranscriptionProvider) -> Ears:
    """Hear on `transcriber` directly: the rig meters nothing, so a fake's `calls` are each clip."""
    bound = BoundTranscriber(
        slot=ModelSlot.TRANSCRIBER, binding="transcriber", provider=transcriber, model="rig-ears"
    )
    return Ears(gate=DirectTranscriptionGate(), bound=bound)
