...
async def test_overriding_footprints_and_grant_ttl_change_the_computed_grant() -> None:
    ...
    2) without zeroing it -- a footprint big enough to zero it is covered by
    `test_a_zero_grant_is_denied_and_fails_the_task_instead_of_being_sent` instead, since a zero
    grant is never sent to the wire at all since this dispatch's own fix -- and a named
    grant_ttl_s lands exactly on the wire grant's own expires_at.
    """
    provider = FakeLLMProvider(responder=plan_responder(_single_task_plan))
