...
def test_an_items_correlation_id_is_its_envelopes_own() -> None:
    """Roadmap step 7.8: a reply's correlation reaches the handler through the InboxItem."""
    clock = FakeClock()
    hive_id, node_id, warden_id = new_hive_id(clock), new_node_id(clock), new_warden_id(clock)
    hop = Hop(sender=warden_id, recipient=hive_id, node_id=node_id)
    answered = new_message_id(clock)
