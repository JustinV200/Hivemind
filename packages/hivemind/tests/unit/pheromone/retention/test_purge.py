...
async def test_side_channels_run_in_their_registered_order_and_seams_are_skipped() -> None:
    calls = list[str]()
    tor, honey, memory, chamber, images, recordings, ledger = (
        _FakeSideChannel(0, calls, name)
        for name in ("tor", "honey", "memory", "chamber", "images", "recordings", "ledger")
    )
    ...
    assert registered == (tor, honey, memory, chamber, images, recordings, ledger)
