"""Unit tests for hivemind.hive.backends.docker.sdk_client.client: the lazy import and pure helpers.

SdkDockerClient's own docker-py calls need a real daemon and are exercised by
packages/hivemind/tests/integration/test_docker_backend.py instead; this module covers what can
be proven with no daemon: the lazy-import guard, the "Created" timestamp parser and the log driver
a Night Veil container is created with.

Fits into the Hive:
    Layer 0 (test infrastructure, not shipped). Mirrors
    src/hivemind/hive/backends/docker/sdk_client/client.py (codingrules section 3).

Key invariants:
    - None: this module holds tests only.

See Also:
    - hivemind.hive.backends.docker.sdk_client.client for SdkDockerClient, under test.
...
from hivemind.hive.backends.docker.sdk_client.client import _log_config, _parse_created_at
