...
# ADR-0039's table, row by row, as the strings a reviewer compares against the ADR itself (with
# phase 6's `exoskeleton` enumerated over its peripherals and phase 7's `honey:read` a glob).
_ADR_TABLE: dict[ScopeKind, set[str]] = {
    ...
    ScopeKind.GLOB: {"tool", "fs:read", "fs:write", "exec", "cell:outside_scratch", "honey:read"},
    ScopeKind.HOST: {"net"},
    ScopeKind.PREFIX: {
        "device",
        "cell:real",
    ...
    ScopeKind.ENUMERATED: {"llm", "cell:comb_shield", "exoskeleton", "tactic"},
...
def test_exoskeleton_scope_values_are_the_four_peripherals() -> None:
    # ADR-0031: a lease-started display, the operator's own screen, audio and the browser.
    assert set(CapabilityFamily.EXOSKELETON.scope_values) == EXOSKELETON_SCOPES
    assert {"display", "real_display", "audio", "browser"} == EXOSKELETON_SCOPES
