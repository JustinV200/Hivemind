...
def test_every_cell_working_role_names_both_honey_tools() -> None:
    """Roadmap step 7.8: `tool:recall`/`tool:remember` survive a ceiling that names its tools."""
    policy = load_guard_policy()

    for role in ("drone", "forager", "scout", "undertaker", "house_bee"):
        allowed = set(role_set(policy, role, _SCRATCH).as_strings())
        assert {"tool:recall", "tool:remember"} <= allowed, role
