...
def test_a_full_warden_holds_its_whole_role_default() -> None:
    warden = warden_set(_POLICY, AccessLevel.FULL, _SCRATCH)

    assert warden == role_set(_POLICY, "warden", _SCRATCH)
    assert not warden.allows(Capability.parse("exoskeleton:real_display"))  # Never a Warden's.
...
def test_warden_set_drops_every_entry_the_deny_list_covers() -> None:
    ...
    warden = warden_set(policy, AccessLevel.FULL, _SCRATCH)

    assert not warden.allows(Capability.parse("exoskeleton"))
