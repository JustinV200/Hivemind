...
in `hive run`'s own view saying why. The first scenario is the one real-task proof (codingrules
section 1's own "prefer a real task over a synthetic one") that the fix still holds one layer up
from `tests.unit.queen.test_queen_dispatch`'s own unit tests: a manifest whose reserve claims every
seat is a lasting shortfall, so the goal must resolve to FAILED with a readable reason in a
handful of seconds, never a timeout.
