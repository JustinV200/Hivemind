...
| 2 | `xvfb` | The virtual framebuffer X server attach starts per lease: `-displayfd` lets it pick a free display itself, `-nolisten tcp` keeps it off the network, and a per-lease MIT cookie (`-auth`) keeps other local users off it. Attach writes that authority file itself (one wildcard entry, so the cookie holds whichever display number Xvfb picks), so no `xauth` is installed. Pulls in Mesa and LLVM, most of this layer's size. |
...
| 5 | `x11-utils` | `xdpyinfo` and `xprop`, for diagnosing a display from `hive cells inspect` or a shell; nothing in the Hive depends on them. |
| 6 | `imagemagick` | `import`, the CompoundEye backend's screen capture: PNG for a frame, raw RGB for a region digest, so comparing a region before and after an action never decodes an image. |
| 7 | `pulseaudio`, `pulseaudio-utils` | The per-lease sound server Buzz starts, with two null sinks: the speaker, whose monitor `listen` records with `parec`, and the microphone, which `say` plays into with `paplay` and whose monitor is the default source applications record from; `pactl` probes readiness. Both sinks load with rewinds disabled (`norewinds=1`): otherwise a monitor holds audio back by up to its two-second rewind window, and a short `listen` hears nothing (found on PulseAudio 16.1 while building phase 6). |
| 8 | `fonts-dejavu-core`, `fonts-liberation` | Legible, metric-compatible text for what a vision model reads off the screen. |
| 9 | `/tmp/.X11-unix`, mode 1777 | The X socket directory, world-writable and sticky as on any desktop, so the unprivileged Xvfb a lease starts can create its socket. |
| 10 | the browser-capable venv | Replaces `base-ubuntu`'s venv at the same path, so every console script's absolute shebang still resolves. |
| 11 | Chromium via `playwright install --with-deps chromium`, in `/opt/playwright-browsers` | Playwright's own Chromium build plus the system libraries and CJK fonts it needs, at a path every user can read (`PLAYWRIGHT_BROWSERS_PATH`), which the Exoskeleton's browser locator scans. Ubuntu 24.04's `chromium` package only installs a snap, which cannot run in a container. The Exoskeleton starts it through the Cell's session with `--no-sandbox` here: the container drops every Linux capability Chromium's own sandbox needs, and the Cell itself is the sandbox (ADR-0031). |
